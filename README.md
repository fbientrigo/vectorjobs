...
> A Colab-first, test-driven pipeline that converts scraper SQLite job postings
...
├── data/
│   ├── bronze/jobs.db       # official scraper SQLite input
│   ├── silver/              # generated
│   └── gold/                # ← NOT committed
├── docs/specs/
...
│   │   ├── load.py          # SQLite bronze → silver writer
...
# 2. Put the official scraper DB at data/bronze/jobs.db

# 3. Build silver dataset
python -m jobsrec.cli build-silver \
    --input-db   data/bronze/jobs.db \
...
# Cell 2 – upload/copy the official scraper DB to data/bronze/jobs.db

# Cell 3 – build silver + TF-IDF + profile
!python -m jobsrec.cli build-silver   --input-db data/bronze/jobs.db --output-dir data/silver --config configs/colab_t4.yaml
...
# Cell 4 – recommend
!python -m jobsrec.cli recommend --job-id 1118215793 --top-k 5
```
...
bronze SQLite and silver Parquet contracts.
...
All tests use **tiny synthetic fixtures** — the full scraper DB is never required
...
Milestone 1 validates the pipeline against synthetic silver data and adds a data
profiling command.
...
- **`tests/test_data_contract.py`** — tiny SQLite fixture covering the bronze-to-silver build.
- **Profile tests** covering profile correctness and legacy timestamp parse helpers.

### Milestone 1 CLI workflow

```bash
# 1. Build silver from bronze SQLite
python -m jobsrec.cli build-silver \
    --input-db data/bronze/jobs.db \
...
|-----------|-------------|
| **M0** ✅ | TF-IDF baseline |
| **M1** ✅ | Bronze SQLite smoke test + data profiling |
